...
public class DatabaseInitializer {
    ...
    @Value("${db.name:support_ping_db}")
    private String databaseName;
    
    @Value("${db.user:support_ping_user}")
    private String dbUser;
    
    @Value("${db.password:ping_secure_pass_2024}")
    ...
    public DataSource dataSource(DataSourceProperties properties) {
        ...
        logger.with("method", "dataSource")
              .info("Initializing database for ping service");
        logger.with("databaseName", databaseName).with("dbUser", dbUser).info("Database configuration");
        ...
        try {
            ...
            try (Connection conn = adminDataSource.getConnection();
                ...
                if (!rs.next()) {
                    logger.with("dbUser", dbUser).info("Creating database user");
                    stmt.execute(String.format(
                        "CREATE USER %s WITH PASSWORD '%s'", 
                        dbUser, dbPassword
                    ));
                } else {
                    logger.with("dbUser", dbUser).info("Database user already exists");
                }
                ...
                if (!rs.next()) {
                    ...
                    logger.with("databaseName", databaseName).info("Creating database");
                    stmt.execute(String.format(
                        "CREATE DATABASE %s OWNER %s", 
                        databaseName, dbUser
                    ));
                    logger.with("databaseName", databaseName).info("Database created successfully");
                } else {
                    logger.with("databaseName", databaseName).info("Database already exists");
                }
                ...
                logger.with("databaseName", databaseName).info("Enabling TimescaleDB extension");
                
                // Need to connect to the actual database to create extension
                ...
                targetDataSource.setUser(adminUsername);  // Use admin for extension creation
                targetDataSource.setPassword(adminPassword);
                
                try (Connection targetConn = targetDataSource.getConnection();
                     Statement targetStmt = targetConn.createStatement()) {
                    targetStmt.execute("CREATE EXTENSION IF NOT EXISTS timescaledb");
                    logger.with("databaseName", databaseName)
                          .with("method", "dataSource")
